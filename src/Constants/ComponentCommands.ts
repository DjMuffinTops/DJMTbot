export const ComponentCommands = {
  SET_DEBUG_CHANNEL: 'setdebugchannel',
  SET_HOURS: 'sethours',
  SET_AUTO_REACT: 'setautoreact',
  PRINT_AUTO_REACT: 'printautoreact',
  SET_BRUH: 'setbruh',
  SET_DOTW: 'setdotw',
  PRINT_DOTW: 'printdotw',
  SET_STAR: 'setstar',
  PRINT_STAR: 'printstar',
  SET_VC_PAIRS: 'setvcpairs',
  PRINT_VC_PAIRS: 'printvcpairs',
  SET_REACT_PAIRS: 'setreactpairs',
  PRINT_REACT_PAIRS: 'printreactpairs',
  DEBUG_MODE: 'debug',
  SET_PREFIX: 'prefix',
  EXPORT_CONFIG: 'exportconfig',
  RESET_CONFIG: 'resetconfig',
  B_SPEAK: 'b',
  BRUH: 'bruh',
  PRINT_BRUH: 'printbruh',
  BRUH_RECACHE: 'rebruh',
  CHEEMS: 'cheems',
  HELP: 'help',
  PING: 'ping',
  SAY: 'say',
  SET_PNGRC: 'setpngrc',
  PRINT_PNGRC: 'printpngrc',
  SET_BANNER: 'setbanner',
  ROTATE_BANNER: 'rotatebanner',
  PRINT_BANNER: 'printbanner',
  SET_MEDIA_CHANNEL: 'setmc',
  PRINT_MEDIA_CHANNEL: 'printmc',
  SET_DYNAMIC_VC: 'setdynamicvc',
  PRINT_DYNAMIC_VC: 'printdynamicvc',
  SET_AUTO_THREAD: 'setautothread',
  PRINT_AUTO_THREAD: 'printautothread',
  SET_MOD_ALERTS_CHANNEL: 'setmodalertschannel',
  SET_MOD_LOGGING_CHANNEL: 'setmodloggingchannel',
  SET_RADIO_VOICE_CHANNEL: 'setradiovoicechannel',
  PERMIT_NEW_USER_RESTRICTIONS: 'permit-new-user-restrictions',
  TOGGLE_NEW_USER_MEDIA_LOCK: 'toggle-new-user-media-lock',
  TOGGLE_NEW_USER_BAN: 'toggle-new-user-ban',
  SET_NEW_USER_MEDIA_LOCK_THRESHOLD_IN_DAYS: 'set-new-user-media-threshold',
  SET_NEW_USER_BAN_THRESHOLD_IN_DAYS: 'set-new-user-ban-threshold',
  TOGGLE_EVERYONE_SPAM_TIMEOUT: 'toggle-everyone-spam-timeout',
  SET_EVERYONE_SPAM_TIMEOUT_ROLES: 'set-everyone-spam-timeout-roles',
  DOG_FACT: 'dogfact',
  CAT_FACT: 'catfact',
  PLAY: 'play',
  RADIO: 'radio',
  RADIO_STATION_ADD: 'radio-station-add',
  RADIO_STATION_DEFAULT: 'radio-station-default',
  RADIO_STATION_LIST: 'radio-station-list',
  RADIO_STATION_REMOVE: 'radio-station-remove',
  RADIO_NOWPLAYING: 'radionowplaying',
  PLAYFILE: 'playfile',
  SKIP: 'skip',
  STOP: 'stop',
  LEAVE: 'leave',
  PAUSE: 'pause',
  RESUME: 'resume',
  QUEUE: 'queue',
  NOWPLAYING: 'nowplaying',
  VOLUME: 'volume',
  LOOP: 'loop',
  AUTOPLAY: 'autoplay',
} as const;

export type ComponentCommand =
  (typeof ComponentCommands)[keyof typeof ComponentCommands];
