...
toggleNewUserMediaLockCommand.setName(
...
setNewUserThresholdInDaysCommand.setName(
...
setNewUserThresholdInDaysCommand.setDefaultMemberPermissions(
  PermissionFlagsBits.Administrator,
);

const setNewUserBanThresholdInDaysCommand = new SlashCommandBuilder();
setNewUserBanThresholdInDaysCommand.setName(
...
setNewUserBanThresholdInDaysCommand.setDefaultMemberPermissions(
  PermissionFlagsBits.Administrator,
);

const permitNewUserRestrictionsCommand = new SlashCommandBuilder();
permitNewUserRestrictionsCommand.setName(
...
export class NewUserProtection extends Component<NewUserProtectionSave> {
  ...
  commands: SlashCommandBuilder[] = [
    permitNewUserRestrictionsCommand,
    toggleNewUserMediaLockCommand,
    toggleNewUserBanCommand,
    setNewUserThresholdInDaysCommand,
    setNewUserBanThresholdInDaysCommand,
  ];
  ...
  async onGuildMemberAdd(member: GuildMember): Promise<void> {
    ...
    if (
      modAlertsChannel &&
      accountAgeInDays <= this.newUserMediaThresholdInDays
    ) {
      await modAlertsChannel.send(
        `⚠️ New discord account <@${member.user.id}> (Created ${accountAgeInDays} days, ${accountAgeInHours} hours, ${accountAgeInMinutes} minutes, ${accountAgeInSeconds} seconds ago) has joined the server.`,
      );
    }

    // If the user is less than the new user ban threshold, ban them
    await this.handleBanNewUser(
      member,
      accountAgeInDays,
      accountAgeInHours,
      accountAgeInMinutes,
  ...
  async onInteractionCreate(interaction: Interaction): Promise<void> {
    ...
    if (
      ...
      await this.djmtGuild.saveJSON();
      await interaction.reply({
        content: `Set new user ban threshold in days to ${this.newUserBanThresholdInDays}.`,
      });
    }
  }
      ...
      if (attemptedMediaPost) {
        ...
        if (modAlertsChannel) {
          const msg1 = await modAlertsChannel.send(
            `⚠️ New discord account <@${user.id}> (${accountAgeInDays} days old) attempted to post the following media in <#${message.channel.id}>`,
          );
        ...
        if (message.channel.isSendable()) {
          ...
          await message.channel.send(
            `Hello, <@${user.id}>! Your account is not permitted to post media due to being a new discord account.\nPlease request to post media by messaging staff through ModMail!`,
          );
        }
      }
      return;
  ...
  private async handleBanNewUser(
    member: GuildMember,
    accountAgeInDays: number,
    accountAgeInHours: number,
    accountAgeInMinutes: number,
    ...
    // If the user is less than the new user ban threshold, ban them
    if (
      !this.permittedUsers.has(member.id) &&
      accountAgeInDays <= this.newUserBanThresholdInDays
    ) {
      await this.banNewUser(
        member,
        accountAgeInDays,
        accountAgeInHours,
        accountAgeInMinutes,
  ...
  private async banNewUser(
    ...
    try {
      await member.ban({
        reason: `New discord account created ${accountAgeInDays} days, ${accountAgeInHours} hours, ${accountAgeInMinutes} minutes, ${accountAgeInSeconds} seconds ago.`,
      });
      // Alert the mod alerts channel of a new user being banned
      if (modAlertsChannel) {
        await modAlertsChannel.send(
          `🚨 Banned new discord account <@${member.user.id}> (Created ${accountAgeInDays} days, ${accountAgeInHours} hours, ${accountAgeInMinutes} minutes, ${accountAgeInSeconds} seconds ago)`,
        );
      }
    } catch (e) {
  ...
  async permitNewUserMedia(
    ...
    if (this.permittedUsers.has(user.id)) {
      ...
      await this.djmtGuild.saveJSON();
      await interaction.reply({
        content: `Removed ${user.toString()} from the list of permitted users.`,
      });
    } else {
      this.permittedUsers.add(user.id);
      await this.djmtGuild.saveJSON();
      await interaction.reply({
        content: `Added ${user.toString()} to the list of permitted users.`,
      });
    }
  }
}
