...
setDVCCommmand.setDescription(
  'Sets a dynamic vc channel. Will create a child vc when occupied.',
);
setDVCCommmand.addChannelOption(input =>
  input
    .setName('voicechannel')
    .setDescription(
      'The channel to add or remove from the dynamic vc channels list',
    )
    .addChannelTypes(ChannelType.GuildVoice)
    .setRequired(true),
);
setDVCCommmand.addIntegerOption(input =>
  input
    .setName('maxchildren')
    .setDescription('The max number of child vcs allowed to be created')
    .setRequired(true),
);
...
export class DynamicVoiceChannels extends Component<DynamicVoiceChannelsSave> {
  ...
  async onReady(): Promise<void> {
    ...
    for (let i = 0; i < rootLength; i++) {
      ...
      const rootVC = this.markedVoiceChannels[i];
      ...
      for (const guildVoiceChannel of allGuildVoiceChannels) {
        ...
        for (
          let nameIndex = 0;
          nameIndex < rootVCInfo.possibleChildrenNames.length;
          nameIndex++
        ) {
          const childVCName = rootVCInfo.possibleChildrenNames[nameIndex];
          ...
          if (childVCName === guildVoiceChannel.name) {
            ...
            try {
              ...
              const newRootDynamicChannel =
                guildVoiceChannel as DynamicVoiceChannel;
              newRootDynamicChannel.root = false;
              newRootDynamicChannel.parentChannel =
                this.markedVoiceChannels.find(
                  voiceChannel =>
                    voiceChannel.name ===
                    rootVCInfo.possibleChildrenNames[nameIndex - 1],
                ) as DynamicVoiceChannel;
              newRootDynamicChannel.childChannel =
                this.markedVoiceChannels.find(
                  voiceChannel =>
                    voiceChannel.name ===
                    rootVCInfo.possibleChildrenNames[nameIndex + 1],
  ...
  async onVoiceStateUpdate(
    ...
    if (
      this.markedVoiceChannels.some(
        markedVC =>
          markedVC.id === oldState.channel?.id ||
          markedVC.id === newState.channel?.id,
      )
    ) {
      await this.markedChannelCreationCheck();
  ...
  private async markedChannelDeletionCheck() {
    ...
    for (let i = this.markedVoiceChannels.length - 1; i >= 0; i--) {
      const markedVC = this.markedVoiceChannels[i];
   ...
   * @param interaction
   */
  async setRootDynamicVoiceChannel(
    voiceChannel: VoiceChannel,
    maxChildren: number,
    interaction: ChatInputCommandInteraction,
  ) {
    const res = await this.addOrRemoveRootDynamicVoiceChannel(
      voiceChannel,
      maxChildren,
    );
  ...
  private async printDyanamicVoiceChannels(
    ...
    const rootChannels = this.markedVoiceChannels.filter(
      markedVC => markedVC.root,
    );
    if (rootChannels.length > 0) {
      let channelString = '';
      rootChannels.forEach((channel: DynamicVoiceChannel) => {
        channelString += `${channel.toString()} => Max Children: ${
          channel.rootsMaxChildren
        }\n`;
      });
   ...
   */
  async addOrRemoveRootDynamicVoiceChannel(
    voiceChannel: VoiceChannel,
    maxChildren?: number,
  ): Promise<string> {
   ...
   */
  async addRootDynamicVoiceChannel(
    voiceChannel: VoiceChannel,
    maxChildren: number,
  ): Promise<void> {
    const newRootDynamicChannel = voiceChannel as DynamicVoiceChannel;
   ...
   */
  async removeRootDynamicVoiceChannel(
    voiceChannel: VoiceChannel,
  ): Promise<boolean> {
    const rootDynamicChannel = voiceChannel as DynamicVoiceChannel;
    for (const rootChannel of this.markedVoiceChannels) {
      if (rootChannel.root && rootChannel.id === rootDynamicChannel.id) {
        this.markedVoiceChannels.splice(
          this.markedVoiceChannels.indexOf(rootChannel),
          1,
        );
        await this.djmtGuild.saveJSON();
        return true;
   ...
   * @private
   */
  private getDynamicVoiceChannelInfo(
    dvc: DynamicVoiceChannel,
  ): DynamicVoiceChannelNameInfo {
    ...
    let rootChannelCount: number =
      lastSpaceIndex === -1
        ? 1
        : Number(rootChannelName.substring(lastSpaceIndex)); // Start generating names from this number
    let rootChannelNameWithoutNumber: string;
    if (isNaN(rootChannelCount)) {
      rootChannelCount = 1;
      rootChannelNameWithoutNumber = rootChannelName;
    } else {
      rootChannelNameWithoutNumber =
        lastSpaceIndex === -1
          ? rootChannelName
          : rootChannelName.substring(0, lastSpaceIndex);
    }
    const possibleChildVCNames: string[] = Array.from(
      Array(dvc.rootsMaxChildren).keys(),
    ).map(
      (x, index) =>
        `${rootChannelNameWithoutNumber} ${index + rootChannelCount + 1}`,
    );
    return {
  ...
  private async createChildVC(voiceChannel: DynamicVoiceChannel) {
    ...
    if (
      ...
      logger.info('Did not generate child channel, at maximum limit', {
        ...
        maxChildren:
          voiceChannel.rootsMaxChildren ||
          this.GUILD_MAXIMUM_GENERATED_CHANNELS,
      });
      return;
    }
    ...
    if (
      this.markedVoiceChannels.some(
        voiceChannel => voiceChannel.name === nextChannelName,
      )
    ) {
      const foundChannel: DynamicVoiceChannel = this.markedVoiceChannels.find(
        voiceChannel => voiceChannel.name === nextChannelName,
      ) as DynamicVoiceChannel;
    ...
    const childVoiceChannel: DynamicVoiceChannel =
      (await this.djmtGuild.guild?.channels.create({
    ...
    await childVoiceChannel.permissionOverwrites.set(
      voiceChannel.permissionOverwrites.cache,
    );
    await childVoiceChannel.setPosition(voiceChannel.position + 1);
    await childVoiceChannel.setUserLimit(
      childVoiceChannel.rootsMaxChildren -
        nextChannelCount +
        this.MINIMUM_NUMBER_OF_OCCUPANTS,
    );
   ...
   * @private
   */
  private async deleteChildVC(
    voiceChannel: DynamicVoiceChannel,
    reason?: string,
  ) {
    ...
    this.markedVoiceChannels.splice(
      this.markedVoiceChannels.indexOf(voiceChannel),
      1,
    );
    try {
