...
setEveryoneSpamTimeoutRolesCommand.setName(
...
export class EveryoneSpamTimeout extends Component<EveryoneSpamTimeoutSave> {
  ...
  commands: SlashCommandBuilder[] = [
    toggleEveryoneSpamTimeoutCommand,
    setEveryoneSpamTimeoutRolesCommand,
  ];
  permittedRoleIds: string[] = [];
  ...
  async onInteractionCreate(interaction: Interaction): Promise<void> {
    ...
    if (
      ...
      if (this.permittedRoleIds.includes(role.id)) {
        ...
        await interaction.reply({
          content: `Role ${role.name} added to the permitted roles list.`,
        });
      }
      await this.djmtGuild.saveJSON();
    }
  ...
  async handleEveryoneSpam(message: Message) {
    ...
    const hasEveryoneOrHerePing =
      message.mentions.everyone ||
      message.content.includes(EVERYONE_PING) ||
      message.content.includes(HERE_PING);
